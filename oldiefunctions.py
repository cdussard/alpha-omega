...
def desired_files(list_files,descriptions,ctrl,patient_name,cote):
    ind_type = 0
    final_files = []
    files_filtered = []
    descr = []
            ...
            if (profondeur in ls_depths):
                for k in range(len(seg_i.spiketrains)):#loop sur les electrodes
                    ...
                    if (seg_i.spiketrains[k].name[9:]==elec):
                        ...
                        sig_filt2 = filter_signal(data_record_i_type_j_elec_k, 1375, 'bandpass', f_range = [4,40],
...
def get_plot(fmin,fmax,files,desc):
